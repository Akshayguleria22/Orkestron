  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
        ...
        <p className="text-sm font-medium text-foreground">{selectedWorkflow.taskInput}</p>
        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
          <span>Intent: <span className="text-foreground capitalize">{selectedWorkflow.intent}</span></span>
      ...
      <div className="flex items-center gap-3">
        {replayStep < 0 ? (
          ...
          <>
            ...
            <button
              onClick={() => !isPlaying && setReplayStep((s) => Math.min(s + 1, totalSteps))}
          ...
          return (
            <div key={node.id} className="relative">
              ...
              >
                ...
                >
                  {isActive ? (
                    <Loader2 className={cn("w-4 h-4 animate-spin", agentColors[node.agent])} />
                  ) : isCompleted ? (
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn(
                    "text-sm font-medium",
                    isActive ? agentColors[node.agent] : isCompleted ? "text-foreground" : "text-zinc-500"
                  )}>
                    {node.agent}
                  </p>
      ...
      <AnimatePresence>
        {replayStep >= totalSteps && (
          ...
          >
            ...
            <p className="text-sm font-medium text-emerald-400">Replay Complete</p>
            {selectedWorkflow.outcome && (
              <p className="text-xs text-muted-foreground mt-1">{selectedWorkflow.outcome}</p>
            )}
          </motion.div>
        )}
      </AnimatePresence>
